        ...
        try {
            ...
            response.getWriter().write(
                    "<html><body><script>" +
                            ...
                            "  window.location.replace('        config.addAllowedOrigin(\"https://assessmatefinal-6cog.vercel.app\");\r\n" + //
                            "/login?error=' + encodeURIComponent(e.message));" +
                            "}" +
                            "</script></body></html>"
            );
    ...
    private void redirectWithError(HttpServletResponse response, String errorMessage) throws IOException {
        ...
        response.getWriter().write(
                "<html><body><script>" +
                        "try {" +
                        "  if (window.opener) {" +
                        "    window.opener.postMessage({type: 'oauth2-error', error: '" + errorMessage + "'}, '        config.addAllowedOrigin(\"https://assessmatefinal-6cog.vercel.app\");\r\n" + //
                        "');" +
                        "    setTimeout(function() { window.close(); }, 500);" +
                        "  } else {" +
                        "    window.location.href = '        config.addAllowedOrigin(\"https://assessmatefinal-6cog.vercel.app\");\r\n" + //
                        "/login?error=" + encodeURIComponent(errorMessage) + "';" +
                        "  }" +
                        "} catch (e) {" +
                        "  console.error('Error in OAuth2 error callback: ' + e.message);" +
                        "  window.location.href = '        config.addAllowedOrigin(\"https://assessmatefinal-6cog.vercel.app\");\r\n" + //
                        "/login?error=' + encodeURIComponent('OAuth2 error callback failed: ' + e.message');" +
                        "}" +
                        "</script></body></html>"
        );
    }
